...
if(transform){return handleVirtualTreeExpand(rows,expanded);}else{return handleBaseTreeExpand(rows,expanded);}}}return(0,_vue.nextTick)();},/**
             ...
             * 如果有滚动条，则滚动到第一行
             */scrollToStartRow(fieldOrColumn){const{afterFullData}=internalData;return $xeTable.scrollToRow(afterFullData[0],fieldOrColumn);},/**
             * 如果有滚动条，则滚动到最后一行
             */scrollToEndRow(fieldOrColumn){const{afterFullData}=internalData;return $xeTable.scrollToRow(afterFullData[afterFullData.length-1],fieldOrColumn);},/**
             * 如果有滚动条，则滚动到对应的列
             ...
             * 如果有滚动条，则滚动到第一列
             */scrollToStartColumn(){const{visibleColumn}=internalData;return $xeTable.scrollToColumn(visibleColumn[0]);},/**
             * 如果有滚动条，则滚动到最后一列
             */scrollToEndColumn(){const{visibleColumn}=internalData;return $xeTable.scrollToColumn(visibleColumn[visibleColumn.length-1]);},/**
             * 手动清除滚动相关信息，还原到初始状态

...
const allDotKeysCache = new WeakMap();
    ...
    switch(res.status){
        ...
        case 404:
            ...
            try {
                ...
                let errRes = await _fetch(`${pkgUrl}_error.log`);
...
function clearParseUrlPkgCache() {
...
async function lookupRange(registry, name, range, unstable, parentUrl) {
    ...
    const lookupPromise = (async ()=>{
        ...
        if (version) {
            ...
            // not found
            const versions = await fetchVersions.call(this, name);
            const semverRange = new SemverRange$2(String(range) || '*');
...
var jspm = /*#__PURE__*/Object.freeze({
    ...
    clearParseUrlPkgCache: clearParseUrlPkgCache,
...
class Resolver {
    ...
    constructor({ env, fetchOpts, providerManager, packageConfigs = {}, preserveSymlinks = false, traceCjs = true, traceTs = true, traceSystem = true }){
        ...
        _define_property$4(this, "visitedUrls", new Set());
...
function setResolution(resolutions, name, installUrl, pkgScope = null) {
    if (pkgScope && !pkgScope.endsWith('/')) throwInternalError(pkgScope);
    if (pkgScope === null) {
   ...
   */ async installTarget(pkgName, { pkgTarget }, mode, pkgScope, parentUrl) {
        ...
        if (pkgScope !== null && this.installs.primary[pkgName]) {
            ...
            if (await this.inRange(primaryUrl, pkgTarget)) {
                this.log('installer/installTarget', `${pkgName} ${pkgScope} -> ${primaryUrl} (primary consolidation)`);
        ...
        if (this.installs.primary[pkgName]) {
            ...
            if (primaryUrl !== pkgUrl) {
                ...
                if (mode !== 'freeze' && this.tryUpgradeAllTo(latestPkg, pkgUrl, installed)) ; else {
    ...
    async installBuiltin(install, specifier, mode, parentUrl) {
        ...
        if (!useLatestPjsonTarget && existingResolution) {
            this.log('installer/install', `existing lock for ${install.name} from ${parentUrl} is ${JSON.stringify(existingResolution)}`);
    ...
    async getBestExistingMatch(matchPkg) {
        ...
        for (const pkg of candidates){
            if (matchPkg.range.has(pkg.version, true)) {
                if (bestMatch) {
                    bestMatch = Semver$1.compare(new Semver$1(bestMatch.version), pkg.version) === -1 ? pkg : bestMatch;
                } else {
                    bestMatch = pkg;
                }
            }
            ...
            if (primaryConstraints[name]) {
                ...
                const target = {
                    pkgTarget: {
                        ...
                        name: pkg.pkg.name,
                        range: new SemverRange$2('^' + pkg.pkg.version),
                        unstable: false
                    }
                };
                installs.push({
